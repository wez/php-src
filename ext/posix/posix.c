...
ZEND_BEGIN_ARG_INFO(arginfo_posix_dup2, 2)
    ZEND_ARG_INFO(0, srcfd)
    ZEND_ARG_INFO(0, destfd)
ZEND_END_ARG_INFO()
...
const zend_function_entry posix_functions[] = {
    ...
    PHP_FE(posix_dup2, arginfo_posix_dup2)
	PHP_FE_END
};
/* }}} */
...
static int get_fd_parameter(zval *val, int *fd TSRMLS_DC)
{
    php_stream *stm;

    php_stream_from_zval_no_verify(stm, &val);
    if (stm) {
        if (FAILURE == php_stream_cast(stm, PHP_STREAM_AS_FD, (void*)fd, 1)) {
            return 0;
        }
        if (*fd == -1) {
            php_error_docref(NULL TSRMLS_CC, E_WARNING, "negative fd value");
            return 0;
        }
        return 1;
    }
    if (Z_TYPE_P(val) != IS_LONG) {
        php_error_docref(NULL TSRMLS_CC, E_WARNING,
                "expected stream or integer parameter for fd");
        return 0;
    }
    *fd = (int)Z_LVAL_P(val);
    if (*fd < 0) {
...
/* {{{ proto bool posix_dup2(int srcfd, int destfd)
   Duplicates srcfd.  If successful, replaces destfd with that duplicate. */
PHP_FUNCTION(posix_dup2)
{
    int res, srcfd = -1, destfd = -1;
    zval *zsrc = NULL, *zdest = NULL;
    ...
    if (!get_fd_parameter(zsrc, &srcfd TSRMLS_CC) ||
            !get_fd_parameter(zdest, &destfd TSRMLS_CC)) {
        RETURN_FALSE;
    }

    res = dup2(srcfd, destfd);
    if (res == destfd) {
        RETURN_TRUE;
    }
    POSIX_G(last_error) = errno;
